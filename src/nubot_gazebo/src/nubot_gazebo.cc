...
void NubotGazebo::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
    // Get the world name.
...
void NubotGazebo::nubot_test(void)
{
    // dribble ball
#if 0
    nubot_locomotion(ignition::math::Vector3d(5,0,0),ignition::math::Vector3d(0,0,2);
            dribble_ball();
    ...
    ROS_FATAL("%d",a);
    nubot_locomotion(ignition::math::Vector3d(0,0,0),ignition::math::Vector3d(0,0,1));
#endif
    //for testing velocity decay
#if 0
    static int count=0;
    ignition::math::Vector3d vel(3,0,0);
    if(count++<50)
    {
        football_model_->SetLinearVel(vel);
        nubot_model_->SetLinearVel(ignition::math::Vector3d(2,0,0));
    }
    debug_msgs_.data.clear();
    double data0 = football_model_->GetWorldLinearVel().Length();
...
#endif
#if 0
    double vel = nubot_model_->GetWorldLinearVel().Length();
    //double vel2 = nubot_state_.twist.linear.x;
    nubot_locomotion(ignition::math::Vector3d(1,0,0),ZERO_VECTOR);
